...
Cyber-Code-Shield 是一个面向企业、内网、离线和高隐私场景的本地 AI 编程助手底座，并逐步演进为本地模型驱动的代码纠错、代码补全和补丁生成工具。

它不是单纯的模型配置文件，也不是一开始就复制完整 Claude Code；当前先帮助用户快速搭建“代码不出本机”的 AI Coding 环境，后续在这个底座上增加类似早期 Cursor 的“一键根据原项目补代码/修代码”能力。

核心组合：

- Ollama：本地运行代码大模型和 embedding 模型
- Continue.dev：在 VS Code 中提供 AI 编程助手体验
- 本地代码库索引：支持离线 `@Codebase` 检索
- 安装/检测脚本：降低普通用户和企业 IT 部署门槛
- 本地代码修复/补丁生成 CLI：基于现有项目上下文生成可审查 diff
...
Cyber-Code-Shield 要提供的是“合规可控的本地 AI 编程能力”，而不是简单宣传模型能力。
...
- 默认不配置云端模型提供商
- 把安装、检测、配置、恢复流程产品化
- 让个人开发者和企业 IT 都能低门槛落地

中期价值主张：

- 用本地模型读取现有项目上下文，辅助定位报错和生成修复建议
- 根据 TODO、空函数、相似模块生成可审查代码补丁
- 先输出 diff 和解释，由用户确认后再写入，避免本地模型直接大范围改代码
- 提供比通用 Continue.dev 配置更明确的“本地代码纠错/补全”工作流
...
- 不承诺绝对安全或绝对零泄漏
- 不一开始做复杂平台
- 不一开始复制完整 Claude Code 的 agent 能力
...
- 企业合规场景的说明、检查和交付材料
- 本地代码纠错、TODO 补全、相似模块补丁生成
- 以可审查 diff 为核心，而不是让本地模型直接大范围自动改项目
