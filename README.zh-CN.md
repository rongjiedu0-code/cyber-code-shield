...
Cyber-Code-Shield 是面向本地 AI Coding 的审查优先审计层。

它帮助你在使用本地代码模型时，把 AI 生成的补丁建议保持为可人工审查、可留痕、可复核的证据。它不是完整的自主编程 Agent，也不会自动应用补丁。

适合：

- 企业内网、离线或强合规研发环境
- Ollama / Continue.dev 本地 AI Coding 工作流
- 需要 policy warnings、已审查文件 hash 和审计证据的团队
- 不能把源码、日志、需求文档或业务上下文发给云端 AI 的开发者

它会生成：

- Markdown 和 JSON 补丁建议报告
- 面向依赖、网络、shell、secret、CI/CD、隐私和供应链复核的 policy warnings
- report ID、模型元数据、prompt/response hash、已审查文件 hash
- v0.6 report bundle：包含机器可读 warning 文件、环境摘要和 manifest 的证据包

> Cyber-Code-Shield 生成本地 AI Coding 复核证据，不是正式合规认证工具。
...
脱敏示例见 [`examples/sample-report-bundle`](examples/sample-report-bundle)。
...
当前项目是早期的本地 AI Coding 审查与治理工具包。v0.6 在 v0.5 架构拆分基础上加入了内置 policy profiles 和 report bundles。
